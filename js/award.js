...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (jsonData && jsonData.length > 0) {
        ...
        document.getElementById('result-container').innerHTML = '<div style="color:#fff; text-align:center">Chưa có dữ liệu</div>';
    }
});
function openDB() {
...
function renderPrizes(winners, config) {
    ...
    config.forEach((prize, index) => {
        ...
        // Tạo danh sách HTML
        const listHtml = prizeWinners.map(winner => `
                    <li class="winner-item">
                        <div class="w-info">
                            <span class="w-name"><span class="icon-gold">${iconDecor}</span>${winner.UserName}</span>
                            <span class="w-dept">${winner.Department || 'Phòng ban'}</span>
                        </div>
