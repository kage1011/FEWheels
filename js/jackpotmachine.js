function createLights(containerId, bulbCount) {
    ...
    const perimeter = topL + (arc * irTR) + rightL + (arc * irBR) + bottomL + (arc * irBL) + leftL + (arc * irTL);
    ...
    for (let i = 0; i < bulbCount; i++) {
        ...
        if (d < topL) {
            ...
            x = iW - irBR - dSub; y = iH;
        } else if (d < topL + arc * irTR + rightL + arc * irBR + bottomL + arc * irBL) {
            // Góc dưới trái
            let angle = Math.PI / 2 + (d - topL - arc * irTR - rightL - arc * irBR - bottomL) / irBL;
            x = irBL + irBL * Math.cos(angle);
            y = (iH - irBL) + irBL * Math.sin(angle);
        } else if (d < perimeter - arc * irTL) {
            // Cạnh trái
            let dSub = d - topL - arc * irTR - rightL - arc * irBR - bottomL - arc * irBL;
            x = 0; y = iH - irBL - dSub;
        } else {
            // Góc trên trái
